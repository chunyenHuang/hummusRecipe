...
exports.text = function text(text = '', x, y, options = {}) {
    ...
    if (textBox.style) {
        ...
        if (!textBox.height) {
            textBox.height = textBox.firstLineHeight * (toWriteTextObjects.length-1)
                           + textBox.paddingTop + textBox.paddingBottom + textBox.lastLineHeight;
    ...
    toWriteTextObjects.forEach((toWriteTextObject, index) => {
        ...
        const writeText = (context, x, y, wto) => {
            ...
            const {lineHeight, lineWidth, text, baseline} = wto;
            ...
            if (options.opacity  === 1 && options.colorspace !== 'separation' &&
               (options.rotation === 0 || options.rotation === undefined)) {
                ...
                const xObject = new xObjectForm(this.writer, lineWidth * 1.5, lineHeight);
                ...
                xObjectCtx
                    .Tf(options.font, options.size)           // set font
