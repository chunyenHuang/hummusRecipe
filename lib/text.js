...
exports.text = function text(text = '', x, y, options = {}) {
    ...
    toWriteTextObjects.forEach((toWriteTextObject, index) => {
            ...
            if (options.opacity !== 1 || options.rotation) {
                ...
                if (options.rotation !== void(0)) {
                    ...
                    const rotateParams = this._getTransformParams(
                        options.rotation, rotationOrigin[0], rotationOrigin[1], x - rotationOrigin[0], y - rotationOrigin[1] - lineHeight);
                    this.pageContext.cm(
        ...
        // There seems to be a problem with centering when spaceWidth is added to currentLine width
        currentLineWidth = currentLineWidth + lineWidth + spaceWidth / 4; // space * writeOptions.size / 14;
