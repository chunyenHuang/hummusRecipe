 ...
 * @param {Object|Boolean} [options.hilite=false] - Used to hilite given text.
...
exports.text = function text(text = '', x, y, options = {}) {
    ...
    const textBox = (isEmpty(options.textBox)) ? {
...
function getFont(textObj, pathOptions) {
    return (textObj.isBold && textObj.isItalic) ? pathOptions.fonts.boldItalic 
         : (textObj.isItalic)                   ? pathOptions.fonts.italic 
         : (textObj.isBold)                     ? pathOptions.fonts.bold : pathOptions.font;
}
...
function makeTextObjects(self, textObject = {}, pathOptions, textBox = {}) {
    ...
    const writeOptions = Object.assign({}, pathOptions, {
        ...
        font           : getFont(textObject, pathOptions)
    });
    ...
    while (bk) {
        ...
        if (flushLine) {
            ...
            if (bk.required) {
                toWriteTextObjects.push(
                    makeTextObject(lines, newLine, lineID, textBox, Object.assign({lineComplete:true}, lineOpts)));
                markLineComplete(toWriteTextObjects);
                newLine = new Line(lineMaxWidth, lineHeight, size, pathOptions);
            }
        }
