...
function _getFontFile(self, options = {}) {
    ...
    if (options.font) {
        fontFile = self.fonts[options.font.toLowerCase()];

        if (!fontFile) {
            fontFile = self.fonts['helvetica'];
        }
        ...
        const font = 
           ((options.bold   || options.isBold) && (options.italic || options.isItalic)) ? 'helvetica-bold-italic' : 
            (options.italic || options.isItalic) ? 'helvetica-italic' : 
            (options.bold   || options.isBold)   ? 'helvetica-bold' : 'helvetica';

        fontFile = self.fonts[font];
    }
