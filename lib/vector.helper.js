exports._getPathOptions = function _getPathOptions(options = {}, originX, originY) {
    const colorspace = this.options.colorspace || 'rgb';
    ...
    const pathOptions = {
        ...
        size: 14,
        underline: false,
        color: transformColor(colorspace),
        colorspace,
        colorArray: [],
        width: 2,
        align: options.align
    ...
    if (options.stroke) {
        pathOptions.stroke = transformColor(colorspace, options.stroke);
    }

    if (options.fill) {
        pathOptions.fill = transformColor(colorspace, options.fill);
    }

    if (options.color || options.colour) {
        pathOptions.color = transformColor(colorspace, (options.color || options.colour));
    }

    pathOptions.colorArray = transformColor(colorspace, (options.color || options.colour), true);
...
function transformColor(colorspace, code = '', returnArray = false) {
    let defaultColor;
    switch (colorspace) {
    case 'cmyk':
    ...
    if (Array.isArray(code)) {
        code = arrayToHexCode(code);
    } else {
        code = code.replace('#', '');
    }

    if (code.toString().length != defaultColor.length) {
        code = defaultColor;
    }

    if (returnArray) {
        return hexToArray(code);
    } else {
