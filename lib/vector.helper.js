...
exports._getPathOptions = function _getPathOptions(options = {}, originX, originY) {
    ...
    } else {  // default font used, now have to make final decision based on bold/italic considerations.
              // Note, if this is not done explicitly, the font dimensions will be incorrect.
         pathOptions.font = 
           (options.bold && options.italic) ? pathOptions.fonts.boldItalic : 
           (options.italic) ? pathOptions.fonts.italic : 
           (options.bold)   ? pathOptions.fonts.bold : pathOptions.font;
    }
