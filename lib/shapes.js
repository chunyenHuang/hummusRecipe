...
function rotate(ox, oy, p, q, angle) {
    ...
    if (angle !== 0) {
        ...
        switch(angle) {
            case   90: 
            case -270: [x,y] = [-y + p,  x + q];
                break;
            case  -90: 
            case  270: [x,y] = [ y + p, -x + q]; 
                break;
            case -180:  
            case  180: [x,y] = [-x + p, -y + q]; 
                break;
            default:
...
exports.triangle = function triangle(x, y, traits, options = {}) {
    ...
    switch(position) {
        case 'centroid':
            ...
            break;
        case 'circumcenter':
            let cc = triangle.circumcenter;
            [pt, radius] = [cc.point, cc.radius];
            break;
        case 'incenter':
            let ic = triangle.incenter;
    ...
    if (options.debug) {
        ...
        if (radius) {
            ...
            ma_A = new Line(triangle.A, triangle.BC.midpoint);
            mb_B = new Line(triangle.B, triangle.AC.midpoint);
            mc_C = new Line(triangle.C, triangle.AB.midpoint);
            this.line([[ma_A.point(1).x, ma_A.point(1).y], [ma_A.point(2).x, ma_A.point(2).y]], {color:'green', width:.5});
            this.line([[mb_B.point(1).x, mb_B.point(1).y], [mb_B.point(2).x, mb_B.point(2).y]], {color:'green', width:.5});
            this.line([[mc_C.point(1).x, mc_C.point(1).y], [mc_C.point(2).x, mc_C.point(2).y]], {color:'green', width:.5});
        }
...
const Triangle = class Triangle {
    constructor(x, y, traitID, traits) {
        ...
        switch(traitID.toLowerCase()) {
            ...
            case 'sss':
                let sss = traits.slice().sort((a,b) => {return a-b});
            ...
            case 'vtx':
                ...
                this._C = traits[1];
                this._A = traits[2];
                let BC = new Line(this._B, this._C);
...
exports.arrow = function arrow(x, y, options = {}) {
    ...
    if (options.double) {
        switch (options.at) {
            case 'head': x -= headLength;  break;
            case 'tail': x += shaftLength + headLength; break;
            default:     x += shaftLength / 2;
        }
        nock = new Kite(x-shaftLength, y, headLength, headWidth, baseOffset);
    } else {
        switch (options.at) {
            case 'head': x -= headLength;  break;
            case 'tail': x += shaftLength; break;
            default:     x += (shaftLength -headLength) / 2;
        }
    }
    ...
    if (options.double) {
        ...
        this.polygon( [
            ...
            arrow.shaft('tr'),  // upper connection point to arrow tip
            arrow.tip.K,
            arrow.tip.I],       // back to point of arrow to close polygon
            options);
    } 
...
const Arrow = class Arrow {
    ...
    shaft(point) {
        switch(point) {
            case 'br': return [this._connectAt_br.x, this._connectAt_br.y];
            case 'bl': return [this._x-this._shaftLength,this._connectAt_br.y];
            case 'tl': return [this._x-this._shaftLength,this._connectAt_tr.y];
            case 'tr': return [this._connectAt_tr.x, this._connectAt_tr.y];
        }
     }
}
...
const Point = class Point {
    ...
    set x(xx)   { this._x = xx }
    set y(yy)   { this._y = yy }
    set point(pnt) { [this._x, this._y] = pnt }
}
...
const Line = class Line {
    ...
    extend(distance, ptNbr=2) {
        ...
        let x = ept.x, y = ept.y;
        let epsilon = .0001;
        let newLen, deltaX=0, deltaY=0;

        switch (slope) {
            case 0:        deltaX = distance; newLen = Math.abs(opt.x - (x+deltaX)); break;
            case Infinity: deltaY = distance; newLen = Math.abs(opt.y - (y+deltaY)); break;
        
            default:
                let ss = Math.sqrt(1/(1+Math.pow(slope,2)));
                deltaX = distance * ss;
                deltaY = slope * deltaX;
    ...
    intersect(CD) {
        let A = this._pt1, B = this._pt2;
        let C = CD, D = CD;
        
        let x1 = A.x, y1 = A.y;
        let x2 = B.x, y2 = B.y;
        
        let x3 = C.point(1).x, y3 = C.point(1).y;
        let x4 = D.point(2).x, y4 = D.point(2).y;
