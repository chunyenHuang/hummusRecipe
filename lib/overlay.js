...
exports.overlay = function overlay(pdfSrc, x = 0, y = 0, options = {}) {
    ...
    const page_index = options.page_index && options.page_index <= inMetadata.pages ? options.page_index : 1
    // for now only handle the first page.
    const { width, height } = inMetadata[page_index];

    this.pauseContext();
    const xObject = new xObjectForm(this.writer, width, height);
    xObject.getContentContext()
        .q()
        .drawImage(0, 0, pdfSrc,{index:page_index - 1})
        .Q();
    xObject.end();
    this.resumeContext();
