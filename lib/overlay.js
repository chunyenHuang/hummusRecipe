 ...
 */
exports.overlay = function overlay(pdfSrc, x = 0, y = 0, options = {},overlay_options = {}) {
    ...
    const { width, height } = inMetadata[(overlay_options.index + 1) || 1];

    this.pauseContext();
    const xObject = new xObjectForm(this.writer, width, height);
    xObject.getContentContext()
        .q()
        .drawImage(0, 0, pdfSrc,overlay_options)
        .Q();
    xObject.end();
    this.resumeContext();
